    ...
    public static class BadaoPoppyAssasinate
    {
        ...
        private static void Drawing_OnDraw(EventArgs args)
        {
            if (!BadaoPoppyHelper.AssasinateActive())
                return;
            var x = LeagueSharp.Drawing.WorldToScreen(ObjectManager.Player.Position);
            var head = LeagueSharp.Drawing.WorldToScreen((ObjectManager.Player.Position.To2D() + new Vector2(0, 250)).To3D());
            var selected = TargetSelector.SelectedTarget;
            if (selected.BadaoIsValidTarget())
            {
                var y = LeagueSharp.Drawing.WorldToScreen(selected.Position);
                LeagueSharp.Drawing.DrawLine(x,y,2,Color.Pink);
                LeagueSharp.Drawing.DrawText(head[0], head[1], Color.Yellow, "selected target is: " + selected.ChampionName);
            }
            else
            {
                LeagueSharp.Drawing.DrawText(head[0], head[1], Color.Yellow, "please select target");
            }
        }
